...
- **主机密钥 TOFU 校验（默认 `safs.hostKeyChangedAction` 为 `prompt`）**：
  - 信任台账保存该主机所有见过的密钥指纹**集合**（负载均衡 VIP / 多台服务器
    共用 IP 时，各后端密钥都收进台账，弹窗自然终结）；
  - **每主机首次连接弹窗确认一次**（TOFU，弹窗突出显示目标主机 IP:端口与
    登录用户，便于确认连的是正确的机器），之后该主机的任何密钥变化（后端
    轮换 / 重装）**静默记录到台账，不再弹窗**（跨会话持久，不打扰日常使用）；
    台账非空即视为已确认——从旧版本静默模式升级的用户不会突然被要求确认；
