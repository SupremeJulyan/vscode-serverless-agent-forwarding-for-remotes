...
- MCP 命令审计日志恢复记录脱敏后的命令原文，同时保留长度和 SHA-256，便于直接审计调用内容。
- 新增 MCP 工具 `remote_edit`，通过唯一精确匹配原子修改工作区内的 UTF-8 文件，
  支持多项事务式编辑和可选 SHA-256 并发校验，避免局部修改时重写整个文件。
...
- Agent 请求列出或切换 SAFS 工作区、主机、配置时，可显式获取全部活动候选，避免
  cwd 精确匹配后反复绑定当前窗口；目标仍需用户明确确认后才会切换。
- Bash Shell Integration 的 cwd hook 现在跨继承的 `PROMPT_COMMAND` 安全运行，
  即使辅助函数不存在也保持静默，并正确保留上一条命令的退出状态。

## 1.6.8

- 远程终端新增文件路径链接：支持 `Ctrl/Cmd+点击` 打开绝对路径、相对路径及
  `文件:行:列`，允许访问当前资源管理器目录之外但仍在挂载根内的文件；越界路径
  只提示新增配置，符号链接仍按真实路径执行边界校验。命令仅输出文件名且直接路径
  不存在时，会在当前工作区内受限查找，唯一匹配直接打开、多项匹配由用户选择。
- 内置 ssh2 终端新增独立的会话级远端 Shell Integration：自动探测 Bash、Zsh、
  Fish，通过 OSC 633 安全上报实时 cwd，执行 `cd` 后相对终端链接立即按新目录解析。
  Bash/Fish 从会话文件描述符加载，不写远端文件；Zsh 使用权限为 `0700` 的临时
  `ZDOTDIR`，加载用户配置后立即自清理，三者均不修改用户的 Shell 配置。
- 为 Windows、Linux 和 macOS 设置各自简洁且避开常见系统冲突的快捷键。

## 1.6.7

- 强化 OpenSSH 主机密钥校验：每次连接都探测并核验当前密钥，兼容 IPv6
  `known_hosts` 条目，固定使用已确认的主机身份，并以原子方式串行更新密钥文件。
- 支持 SSH 负载节点轮换：用户选择“追加新密钥”后记住该主机为负载节点，后续发现
  新后端密钥时自动追加并安全重试；服务器重装仍可选择替换旧密钥。
- 精简主机密钥确认界面，移除与“取消”重复的拒绝操作，并明确追加、替换的适用场景。
- Agent 转发安装结果只报告实际检测到的 Agent；设置说明增加自定义 Agent 的手工安装
  命令，且 SAFS 配置文件路径固定使用扩展默认值。
- 抽取 MCP 工具定义、命令策略与 POSIX Shell 转义逻辑，删除重复实现并补充测试覆盖。
...
- Linux、macOS、WSL（`vpn: false`）的密码终端改为优先使用内置 ssh2，与 Windows
  保持一致：主机密钥由实际终端连接直接确认，不再先用 `ssh-keyscan` 探测、再由
  系统 SSH 建立另一条连接，避免首次使用或负载均衡后端轮换时“目录已连接、终端
  仍需重试数次”。WSL VPN、中继、私钥及不兼容 ssh2 shell 的回退仍使用系统 SSH。
